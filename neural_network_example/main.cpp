...
int main() {
		...
		Dataset image_training_data = Dataset("train-images.idx3-ubyte", "train-labels.idx1-ubyte", 100);
    	...
    	//train networks
    	plain_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.001, 10, 0.0, new L2(0.1), 100, false, new No_Optimization());
		momentum_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.001, 10, 0.0, new L2(0.1), 100, false, new Momentum(0.1));
    	rms_prop_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.001, 10, 0.0, new L2(0.1), 100, false, new RMS_Prop(0.9));
		adam_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.001, 10, 0.0, new L2(0.1), 100, false, new Adam());
