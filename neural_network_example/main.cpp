...
int main() {
	...
	std::unique_ptr<Dataset> image_test_data = std::make_unique<Dataset>(Dataset("t10k-images.idx3-ubyte", "t10k-labels.idx1-ubyte"));
    	...
    	//train network
    	model.train_network(std::move(image_training_data), 0.001, 1, 0.0, new L2(0.1), 256, false, new No_Optimization());
