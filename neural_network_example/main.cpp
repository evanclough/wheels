...
int main() {
		...
		Dataset image_training_data = Dataset("train-images.idx3-ubyte", "train-labels.idx1-ubyte", 200);
		Dataset image_test_data = Dataset("t10k-images.idx3-ubyte", "t10k-labels.idx1-ubyte", 100);
    	...
    	//train networks
    	plain_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.01, 10, 0.0, new L2(0.1), 100, false, new No_Optimization());
		momentum_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.01, 10, 0.0, new L2(0.1), 100, false, new Momentum(0.1));
    	rms_prop_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.01, 10, 0.0, new L2(0.1), 100, false, new RMS_Prop(0.9));
		adam_model.train_network(std::move(std::make_unique<Dataset>(image_training_data)), 0.1, 10, 0.0, new L2(0.1), 100, false, new Adam());
	...
	for(int i = 0; i < models.size(); i++){
		...
		for(int j = 0; j < test_feature_data.size(); j++){
			inferences.push_back(models[i]->inference(test_feature_data[j]));
		}

		float accuracy = 0;
		for(int j = 0; j < test_label_data.size(); j++){
			bool same = false;
			for(int k = 0; k < 10; k++){
		...
		std::cout << "Test Accuracy of " << models[i]->get_model_name() << ": " << accuracy << std::endl;
	}
}
